...
class CozinhaApp:
    ...
    def build_login(self):
        ...
        ctk.CTkLabel(frame, text="OrderStack", font=ctk.CTkFont("Syne", 36, "bold"),
                     text_color="#ff4500").pack(pady=(0,2))
        ctk.CTkLabel(frame, text="COZINHA", font=ctk.CTkFont("Syne", 11),
                     text_color="#333333").pack()
        ctk.CTkLabel(frame, text="", fg_color="transparent").pack(pady=14)

        ctk.CTkLabel(frame, text="EMAIL", font=ctk.CTkFont(size=10),
                     text_color="#555555").pack(anchor="w")
        self.email_var = ctk.StringVar()
        ctk.CTkEntry(frame, textvariable=self.email_var, width=320, height=44,
                     font=ctk.CTkFont(size=13), fg_color="#111111",
                     border_color="#222222", border_width=1,
                     corner_radius=8).pack(pady=(4,12))

        ctk.CTkLabel(frame, text="SENHA", font=ctk.CTkFont(size=10),
                     text_color="#555555").pack(anchor="w")
        self.senha_var = ctk.StringVar()
        senha_entry = ctk.CTkEntry(frame, textvariable=self.senha_var, show="*",
                                   width=320, height=44, font=ctk.CTkFont(size=13),
                                   fg_color="#111111", border_color="#222222",
                                   border_width=1, corner_radius=8)
        senha_entry.pack(pady=(4,24))
        senha_entry.bind("<Return>", lambda e: self.fazer_login())

        self.btn_login = ctk.CTkButton(frame, text="CONECTAR", command=self.fazer_login,
                                       width=320, height=48, font=ctk.CTkFont("Syne", 13, "bold"),
                                       fg_color="#ff4500", hover_color="#ff5a1f",
                                       corner_radius=8)
        self.btn_login.pack()

        self.status_var = ctk.StringVar(value="")
        ctk.CTkLabel(frame, textvariable=self.status_var, font=ctk.CTkFont(size=11),
                     text_color="#ff4500", wraplength=300).pack(pady=12)
        ...
        try:
            ...
            if res.ok:
                ...
                rest_res = requests.get(
                    f"{self.BASE_URL}/dashboard/restaurant/{self.restaurant_id}/info",
    ...
    def build_cozinha(self):
        ...
        ctk.CTkLabel(header, text="OrderStack Cozinha",
                     font=ctk.CTkFont("Syne", 16, "bold"),
                     text_color="#ff4500").place(relx=0.5, rely=0.38, anchor="center")
        ctk.CTkLabel(header, text=self.restaurant_name or "Restaurante",
                     font=ctk.CTkFont(size=10),
                     text_color="#444444").place(relx=0.5, rely=0.72, anchor="center")
        ...
        self.live_label = ctk.CTkLabel(status_frame, text="● ONLINE",
                                       font=ctk.CTkFont(size=11, weight="bold"),
                                       text_color="#00d97e")
        self.live_label.pack(side="left")

        self.ultima_var = ctk.StringVar(value="Aguardando pedidos...")
        ctk.CTkLabel(status_frame, textvariable=self.ultima_var,
                     font=ctk.CTkFont(size=10), text_color="#333333").pack(side="right")

        # Label pedidos
        ctk.CTkLabel(self.root, text="PEDIDOS RECEBIDOS",
                     font=ctk.CTkFont(size=9), text_color="#333333").pack(anchor="w", padx=16, pady=(10,4))

        # Area de pedidos
        self.lista = ctk.CTkTextbox(self.root, fg_color="#0d0d0d", text_color="#efefef",
                                    font=ctk.CTkFont("Courier New", 11),
                                    corner_radius=8, border_width=1,
                                    border_color="#1a1a1a", wrap="word",
                                    state="disabled")
        self.lista.pack(fill="both", expand=True, padx=16, pady=(0,8))

        # Tags
        self.lista._textbox.tag_config("titulo", foreground="#ff4500",
                                        font=("Courier New", 12, "bold"))
        self.lista._textbox.tag_config("item", foreground="#efefef")
        self.lista._textbox.tag_config("total", foreground="#00d97e",
        ...
        ctk.CTkButton(btn_frame, text="Sair da conta", command=self.sair,
                      width=120, height=32, font=ctk.CTkFont(size=11),
                      fg_color="#1a1a1a", hover_color="#222222",
                      text_color="#555555", corner_radius=6).pack(side="right")
            ...
            try:
                ...
                if res.ok:
                    ...
                    self.root.after(0, lambda: self.live_label.configure(
                        text="● ONLINE", text_color="#00d97e"))
                    self.root.after(0, lambda: self.ultima_var.set(
                    ...
                    self.root.after(0, lambda: self.live_label.configure(
                        text="● ERRO", text_color="#ff3b3b"))
            except:
                self.root.after(0, lambda: self.live_label.configure(
                    text="● OFFLINE", text_color="#ff3b3b"))
            time.sleep(3)
    ...
    def mostrar_pedido(self, pedido):
        ...
        for item in pedido.get("items", []):
            self.lista._textbox.insert("end",
                f"  {item['quantity']}x  Produto #{item['product_id']}  R$ {item['unit_price']:.2f}\n", "item")
        self.lista._textbox.insert("end", "-" * 36 + "\n", "sep")
    ...
    def _marcar_impresso(self, order_id):
        try:
            requests.patch(f"{self.BASE_URL}/orders/{order_id}/impresso",
                           headers={"Authorization": f"Bearer {self.token}"}, timeout=10)
        except:
            pass
